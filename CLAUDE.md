...
**Couche `/api`** (`api/*.js` = 1 route par fichier ; `api/_lib/*` = librairies partagées, `_data/*` = données ; préfixe `_` exclu du routage) : publish/restore/history (publication GitHub), perf/perf-history (PageSpeed ; `?run=1` = mesure planifiée), chat (RAG+LLM), calendly, config, health (+`?probe=env`), media-upload (Blob), collect (audience). Auth = **jeton de session Clerk vérifié serveur (JWKS)** OU repli `PUBLISH_SECRET`, puis **RBAC par capacité** (`api/_lib/rbac.js`, 403 sinon). Stockage d'objets derrière la couture `api/_lib/storage.js` (Vercel Blob REST → Azure). Contrat de contenu publié : `api/_lib/content-schema.js`. **⚠️ Plan Vercel Hobby = 12 Functions max** (consolidations pour tenir : env-check→health, perf-cron→perf-history). Voir `docs/` (un fichier par chantier) et `README.md`.
