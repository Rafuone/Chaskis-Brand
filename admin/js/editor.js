...
const ADMIN_BUILD = { version: "0.16.14" };
...
const RELEASE_LOG=[
  { v:"v0.16.14", cur:true, date:"2026-07-08", title:"Base de la publication : le site sait lire le contenu publié", items:[
...
const TECH_FEATURES=[
  ...
  { group:"Statistiques & Performance", items:[
    ...
    {n:"Performance branchée sur PageSpeed / Lighthouse", s:"todo"} ]},
...
const TECH_DONE={host:70,publish:30,versioning:28,analytics:30,calendly:25,auth:25,perf:30,media:20,chatbot:20};
...
const TECH_COST_SHORT={host:"Gratuit",publish:"Gratuit",versioning:"Gratuit",auth:"Gratuit",analytics:"Gratuit",perf:"Gratuit",chatbot:"0 à 15 CHF/mois",calendly:"Gratuit ou ~12 CHF/mois",media:"Gratuit"};
...
const TECH_BRIEF={
  ...
  perf:{ sum:"Mesurer pour de vrai la vitesse et le référencement du site.",
    today:"La note de performance est un exemple, pas une vraie mesure.",
  ...
  calendly:{ sum:"Brancher les vrais rendez-vous Calendly dans l'admin.",
    today:"Les rendez-vous affichés sont des exemples, et le formulaire du site n'ouvre pas encore Calendly.",
    goal:"La prise de rendez-vous passe par Calendly, et les rendez-vous remontent dans l'admin.",
    cost:"Réservation gratuite. Remontée automatique : environ 12 CHF par mois et par page de réservation.",
    note:"Les personnes qui utilisent l'admin ne paient rien. L'abonnement Calendly se compte par page de réservation, pas par utilisateur : une page centrale = un seul abonnement. Il ne sert qu'à la remontée automatique ; la prise de rendez-vous, elle, est gratuite tout de suite.",
...
function renderPerfVerdict(overall){ const w=document.getElementById("perfVerdict"); if(!w) return;
  ...
  const goodN=PERF_PILLARS.filter(p=>p.score>=80).length, actN=PERF_ACTIONS.length;
  const sub=goodN+" domaine"+(goodN>1?"s":"")+" au vert sur "+PERF_PILLARS.length+", "+actN+" point"+(actN>1?"s":"")+" à améliorer, expliqués et guidés plus bas.";
...
function renderPerfPillars(){ const w=document.getElementById("perfPillars"); if(!w) return; w.innerHTML="";
  PERF_PILLARS.forEach(p=>{ const b=pfScore(p.score); const el=document.createElement("div"); el.className="pcard";
...
function renderPerfActions(){ const w=document.getElementById("perfActions"); if(!w) return; const hint=document.getElementById("perfActionsHint");
  const acts=PERF_ACTIONS.slice().sort((a,b)=>(PF_IMPACT_ORDER[a.impact]-PF_IMPACT_ORDER[b.impact]));
...
function renderPerfContent(){ const w=document.getElementById("perfContent"); if(!w) return; const detailed=perfContentMode==="detailed";
  ...
  const ordered=PERF_CONTENT.slice().sort((a,b)=>(a.st==="good"?1:0)-(b.st==="good"?1:0));
